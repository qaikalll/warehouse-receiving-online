(function(){
  ...
  const FEATURE_ID='wrs-receiving-tv-board-v3';
  ...
  function statusOf(r){
    if(r.completionTime)return 'Complete';
  ...
  function slotStart(r){
    const direct=String(r.bookingSlotStart||'').trim();
    if(direct)return direct;
    const m=String(r.bookingSlot||'').match(/(\d{1,2}):(\d{2})/);
  ...
  function bookingDueMs(r){
    const date=String(r.shipmentDate||'').trim();
    if(!/^\d{4}-\d{2}-\d{2}$/.test(date))return 0;
    const time=slotStart(r);
    const raw=time?`${date}T${time}:00`:`${date}T23:59:00`;
    const ms=Date.parse(raw);
    return Number.isFinite(ms)?ms:0;
  }

  function isOverdueScheduled(r,nowMs=Date.now()){
    if(statusOf(r)!=='Scheduled')return false;
    const due=bookingDueMs(r);
    return !!due&&nowMs>due;
  }

  function isLateArrival(r){
    const due=bookingDueMs(r);
    const arrived=parseMs(r.arrivalTime);
    return !!due&&!!arrived&&arrived>due;
  }
  ...
  function shortTime(value){
    const ms=parseMs(value);
    if(!ms)return '-';
    return new Date(ms).toLocaleTimeString('en-MY',{hour:'2-digit',minute:'2-digit',hour12:true});
  }

  function bookedLabel(r){
    const slot=r.bookingSlot||slotStart(r)||'-';
    return `${shortDate(r.shipmentDate)} · ${slot}`;
  }

  function lateInfo(r){
    const due=bookingDueMs(r);
    if(!due)return null;
    if(statusOf(r)==='Scheduled'&&isOverdueScheduled(r)){
      return {
        type:'delayed',
        title:'DELAYED',
        line:`Booked ${bookedLabel(r)} · Overdue +${formatDelay(Date.now()-due)}`
      };
    }
    if(isLateArrival(r)){
      const arrived=parseMs(r.arrivalTime);
      return {
        type:'late',
        title:'LATE ARRIVAL',
        line:`Booked ${bookedLabel(r)} · Arrived ${shortDate(r.arrivalTime)} ${shortTime(r.arrivalTime)} · Delay +${formatDelay(arrived-due)}`
      };
    }
    return null;
  }

  function visibleToday(r,today){
    const status=statusOf(r);
    const bookingDate=String(r.shipmentDate||'');
    if(status==='Complete'){
      const completed=parseMs(r.completionTime);
      return !!completed&&localDateISO(new Date(completed))===today;
    }
    if(status==='Scheduled'){
      return bookingDate===today||(bookingDate&&bookingDate<today&&isOverdueScheduled(r));
    }
    if(status==='Arrived'||status==='Receiving'){
  ...
  function injectStyles(){
    ...
    style.textContent=`
      ...
      .wrs-tv-summary{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;padding:14px 28px 0}.wrs-tv-summary-card{border:1px solid rgba(255,255,255,.10);border-radius:14px;padding:10px 14px;display:flex;align-items:center;justify-content:space-between;background:rgba(255,255,255,.045)}.wrs-tv-summary-card span{font-size:12px;font-weight:900;letter-spacing:.08em;text-transform:uppercase;opacity:.72}.wrs-tv-summary-card strong{font-size:26px}
      .wrs-tv-delay-alert{display:none;margin:12px 28px 0;padding:10px 14px;border:1px solid rgba(239,68,68,.45);background:rgba(127,29,29,.28);color:#fecaca;border-radius:12px;font-size:13px;font-weight:900;letter-spacing:.03em}
      .wrs-tv-delay-alert.show{display:block}
      .wrs-tv-columns{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;padding:16px 28px 24px;min-height:0;flex:1}
      ...
      .wrs-tv-lane-head{display:flex;align-items:center;justify-content:space-between;padding:16px 18px;border-bottom:1px solid rgba(255,255,255,.09)}.wrs-tv-lane-head strong{font-size:17px;letter-spacing:.04em}.wrs-tv-lane-head span{min-width:34px;height:28px;padding:0 9px;border-radius:999px;display:inline-flex;align-items:center;justify-content:center;background:rgba(255,255,255,.11);font-weight:950}
      .wrs-tv-lane.scheduled .wrs-tv-lane-head{background:rgba(59,130,246,.14)}.wrs-tv-lane.arrived .wrs-tv-lane-head{background:rgba(249,115,22,.14)}.wrs-tv-lane.receiving .wrs-tv-lane-head{background:rgba(245,158,11,.14)}.wrs-tv-lane.complete .wrs-tv-lane-head{background:rgba(16,185,129,.14)}
      .wrs-tv-list{padding:12px;display:flex;flex-direction:column;gap:10px;overflow:auto;min-height:0;scrollbar-width:thin}
      .wrs-tv-card{position:relative;border:1px solid rgba(255,255,255,.10);background:#111a2f;border-radius:16px;padding:14px 15px 13px;box-shadow:0 12px 30px rgba(0,0,0,.16)}.wrs-tv-card.next{outline:2px solid rgba(96,165,250,.75);box-shadow:0 0 0 5px rgba(96,165,250,.10),0 12px 30px rgba(0,0,0,.18)}.wrs-tv-card.is-delayed{border-color:rgba(239,68,68,.52);box-shadow:0 0 0 2px rgba(239,68,68,.08),0 12px 30px rgba(0,0,0,.18)}
      .wrs-tv-card-top{display:flex;gap:12px;align-items:flex-start;justify-content:space-between}.wrs-tv-time{font-size:18px;font-weight:950;line-height:1.1}.wrs-tv-status-wrap{display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end}.wrs-tv-status{font-size:10px;font-weight:950;letter-spacing:.07em;text-transform:uppercase;border-radius:999px;padding:6px 8px;white-space:nowrap}.wrs-tv-status.scheduled{background:rgba(59,130,246,.16);color:#93c5fd}.wrs-tv-status.arrived{background:rgba(249,115,22,.16);color:#fdba74}.wrs-tv-status.receiving{background:rgba(245,158,11,.16);color:#fde68a}.wrs-tv-status.complete{background:rgba(16,185,129,.16);color:#6ee7b7}.wrs-tv-status.delayed,.wrs-tv-status.late{background:rgba(239,68,68,.18);color:#fca5a5}
      .wrs-tv-company{font-size:21px;line-height:1.05;font-weight:950;margin-top:9px;overflow-wrap:anywhere}.wrs-tv-do{font-size:14px;font-weight:850;margin-top:7px;opacity:.94}.wrs-tv-meta{display:grid;grid-template-columns:1fr 1fr;gap:7px 10px;margin-top:10px}.wrs-tv-meta span{font-size:12px;opacity:.72;overflow-wrap:anywhere}.wrs-tv-meta b{font-weight:900;color:#fff;opacity:1}
      .wrs-tv-delay-note{margin-top:10px;padding:9px 10px;border-radius:10px;background:rgba(127,29,29,.28);border:1px solid rgba(239,68,68,.28);color:#fecaca}.wrs-tv-delay-note strong{display:block;font-size:11px;letter-spacing:.06em;margin-bottom:3px}.wrs-tv-delay-note span{font-size:11px;line-height:1.35}
      .wrs-tv-note{margin-top:9px;padding-top:8px;border-top:1px solid rgba(255,255,255,.08);font-size:11px;opacity:.66;line-height:1.35}
      ...
      @media(max-width:1250px){.wrs-tv-top{grid-template-columns:1fr auto}.wrs-tv-next{grid-column:1/-1}.wrs-tv-columns{gap:10px;padding-left:14px;padding-right:14px}.wrs-tv-summary,.wrs-tv-delay-alert{margin-left:14px;margin-right:14px}.wrs-tv-summary{padding-left:0;padding-right:0}.wrs-tv-company{font-size:18px}.wrs-tv-time{font-size:16px}}
      @media(max-width:900px){#wrsReceivingTvBoard{overflow:auto}.wrs-tv-top{grid-template-columns:1fr;padding:18px}.wrs-tv-clock{text-align:left}.wrs-tv-actions{justify-content:flex-start}.wrs-tv-summary{grid-template-columns:repeat(2,1fr);padding:12px 18px 0;margin:0}.wrs-tv-delay-alert{margin:12px 18px 0}.wrs-tv-columns{grid-template-columns:repeat(2,1fr);padding:12px 18px 22px;overflow:visible}.wrs-tv-lane{min-height:300px}.wrs-tv-title h1{font-size:28px}}
      @media(max-width:620px){.wrs-tv-summary,.wrs-tv-columns{grid-template-columns:1fr}}
    `;
  ...
  function mountBoard(){
    ...
    board.innerHTML=`
      ...
      <div class="wrs-tv-summary">
        <div class="wrs-tv-summary-card"><span>Scheduled</span><strong id="wrsTvCountScheduled">0</strong></div>
        <div class="wrs-tv-summary-card"><span>Arrived</span><strong id="wrsTvCountArrived">0</strong></div>
        <div class="wrs-tv-summary-card"><span>Receiving</span><strong id="wrsTvCountReceiving">0</strong></div>
        <div class="wrs-tv-summary-card"><span>Complete</span><strong id="wrsTvCountComplete">0</strong></div>
      </div>
      <div class="wrs-tv-delay-alert" id="wrsTvDelayAlert"></div>
      <div class="wrs-tv-columns">
        <section class="wrs-tv-lane scheduled"><div class="wrs-tv-lane-head"><strong>SCHEDULED</strong><span id="wrsTvLaneCountScheduled">0</span></div><div class="wrs-tv-list" id="wrsTvListScheduled"></div></section>
        <section class="wrs-tv-lane arrived"><div class="wrs-tv-lane-head"><strong>ARRIVED</strong><span id="wrsTvLaneCountArrived">0</span></div><div class="wrs-tv-list" id="wrsTvListArrived"></div></section>
        ...
        <section class="wrs-tv-lane complete"><div class="wrs-tv-lane-head"><strong>COMPLETE</strong><span id="wrsTvLaneCountComplete">0</span></div><div class="wrs-tv-list" id="wrsTvListComplete"></div></section>
      </div>`;
  ...
  function cardHtml(r,nextId){
    const status=statusOf(r),isNext=String(r.id)===String(nextId);
    ...
    const late=lateInfo(r);
    const delayBadge=late?`<span class="wrs-tv-status ${late.type==='delayed'?'delayed':'late'}">${esc(late.title)}</span>`:'';
    const delayNote=late?`<div class="wrs-tv-delay-note"><strong>${esc(late.title)}</strong><span>${esc(late.line)}</span></div>`:'';
    return `<article class="wrs-tv-card ${isNext?'next':''} ${late?'is-delayed':''}" data-booking-id="${esc(r.id||'')}">
      <div class="wrs-tv-card-top"><div class="wrs-tv-time">${esc(time)}</div><div class="wrs-tv-status-wrap"><span class="wrs-tv-status ${esc(statusClass)}">${esc(status)}</span>${delayBadge}</div></div>
  ...
  function render(){
    ...
    const bookings=state.bookings.filter(r=>visibleToday(r,today)).sort(compareBySlot);
    const scheduled=bookings.filter(r=>laneOf(r)==='scheduled');
    const arrived=bookings.filter(r=>laneOf(r)==='arrived');
    const receiving=bookings.filter(r=>laneOf(r)==='receiving');
    const complete=bookings.filter(r=>laneOf(r)==='complete');

    const now=nowMinutes();
    const normalScheduled=scheduled.filter(r=>!isOverdueScheduled(r));
    const next=normalScheduled.find(r=>r.shipmentDate===today&&(()=>{const m=timeToMinutes(slotStart(r));return m!==null&&m>=now})())||normalScheduled.find(r=>r.shipmentDate===today)||null;
    ...
    $('wrsTvCountScheduled').textContent=String(scheduled.length);$('wrsTvLaneCountScheduled').textContent=String(scheduled.length);
    $('wrsTvCountArrived').textContent=String(arrived.length);$('wrsTvLaneCountArrived').textContent=String(arrived.length);
    $('wrsTvCountReceiving').textContent=String(receiving.length);$('wrsTvLaneCountReceiving').textContent=String(receiving.length);
    $('wrsTvCountComplete').textContent=String(complete.length);$('wrsTvLaneCountComplete').textContent=String(complete.length);

    const delayedCount=bookings.filter(r=>isOverdueScheduled(r)||isLateArrival(r)).length;
    const alert=$('wrsTvDelayAlert');
    if(delayedCount){
      alert.textContent=`⚠ ${delayedCount} DELAYED SHIPMENT${delayedCount===1?'':'S'} — late arrivals are marked automatically.`;
    ...
    if(next){
      ...
      $('wrsTvNextSub').textContent=scheduled.some(r=>isOverdueScheduled(r))?'Delayed shipment(s) are still waiting to arrive.':'No upcoming scheduled arrival for today.';
    }

    $('wrsTvListScheduled').innerHTML=scheduled.length?scheduled.map(r=>cardHtml(r,nextId)).join(''):'<div class="wrs-tv-empty">No scheduled booking.</div>';
    $('wrsTvListArrived').innerHTML=arrived.length?arrived.map(r=>cardHtml(r,'')).join(''):'<div class="wrs-tv-empty">No arrived shipment.</div>';
    ...
    $('wrsTvListComplete').innerHTML=complete.length?complete.map(r=>cardHtml(r,'')).join(''):'<div class="wrs-tv-empty">No completed receiving yet.</div>';
  }
