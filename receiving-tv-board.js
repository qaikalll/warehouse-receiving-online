(function(){
  ...
  const FEATURE_ID='wrs-receiving-tv-board-v2';
  ...
  function injectStyles(){
    ...
    style.textContent=`
      ...
      @media(max-width:1250px){.wrs-tv-top{grid-template-columns:1fr auto}.wrs-tv-next{grid-column:1/-1}.wrs-tv-columns{gap:10px;padding-left:14px;padding-right:14px}.wrs-tv-summary{padding-left:14px;padding-right:14px}.wrs-tv-company{font-size:18px}.wrs-tv-time{font-size:16px}}
      @media(max-width:900px){#wrsReceivingTvBoard{overflow:auto}.wrs-tv-top{grid-template-columns:1fr;padding:18px}.wrs-tv-clock{text-align:left}.wrs-tv-actions{justify-content:flex-start}.wrs-tv-summary{grid-template-columns:repeat(2,1fr);padding:12px 18px 0}.wrs-tv-columns{grid-template-columns:repeat(2,1fr);padding:12px 18px 22px;overflow:visible}.wrs-tv-lane{min-height:300px}.wrs-tv-title h1{font-size:28px}}
  ...
  function render(){
    ...
    const next=scheduled.find(r=>{const m=timeToMinutes(slotStart(r));return m!==null&&m>=now})||scheduled[0]||null;
    ...
    if(next){
      ...
      $('wrsTvNextSub').textContent=bookings.length?'All scheduled arrivals are already arrived, receiving or complete.':'No booking scheduled for today.';
    }
